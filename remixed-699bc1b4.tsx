...
import { Plus, Trash2, ShoppingCart, Settings, ChefHat, Target, Sparkles, Dumbbell, Sofa, Calendar, ChevronDown, ChevronUp, Copy } from 'lucide-react';
...
const MACRO_LABELS = { calories: 'Calorieën', protein: 'Eiwit (g)', carbs: 'Koolhydraten (g)', fats: 'Vetten (g)' };
const MACRO_LABELS_SHORT = { calories: 'Cal', protein: 'P (g)', carbs: 'C (g)', fats: 'F (g)' };
...
const FitnessMealPlanner = () => {
  ...
  const [expandedIngCat, setExpandedIngCat] = useState(null);
  ...
  const toggleTraining = (dayIdx) => {
    const u = [...dayConfigs];
    u[dayIdx] = { ...u[dayIdx], isTraining: !u[dayIdx].isTraining };
    setDayConfigs(u);
  };

  const toggleCustomMacros = (dayIdx) => {
    const u = [...dayConfigs];
    const c = u[dayIdx];
  ...
  const updateCustomMacro = (dayIdx, field, value) => {
    const u = [...dayConfigs];
    u[dayIdx] = { ...u[dayIdx], customMacros: { ...u[dayIdx].customMacros, [field]: parseInt(value) || 0 } };
    setDayConfigs(u);
  };

  const updateBaseMacro = (type, field, val) => {
  ...
  const mealTemplates = [
    { name: 'Ontbijt', cats: ['Protein', 'Carbs', 'Fruit'] },
    { name: 'Snack 1', cats: ['Protein', 'Fats'] },
    { name: 'Lunch', cats: ['Protein', 'Carbs', 'Veggies'] },
    { name: 'Pre-Workout', cats: ['Protein', 'Carbs'] },
    { name: 'Post-Workout', cats: ['Protein', 'Carbs', 'Veggies'] },
    { name: 'Avondeten', cats: ['Protein', 'Carbs', 'Fats', 'Veggies'] },
    { name: 'Snack Avond', cats: ['Protein', 'Fats'] },
    { name: 'Extra Maaltijd', cats: ['Protein', 'Carbs'] }
  ];

  const generatePlan = (dayIdx) => {
    ...
    for (let i = 0; i < mealsPerDay; i++) {
      const tmpl = mealTemplates[i] || { name: `Maaltijd ${i + 1}`, cats: ['Protein', 'Carbs'] };
      const foods = [];
  ...
  const addFoodToMeal = (dayIdx, mealId, ing) => {
    const u = { ...mealPlans };
  ...
  const removeFoodFromMeal = (dayIdx, mealId, fi) => {
    const u = { ...mealPlans };
    const meals = [...(u[dayIdx] || [])];
    const meal = meals.find(m => m.id === mealId);
  ...
  const updateFoodQty = (dayIdx, mealId, fi, qty) => {
    const u = { ...mealPlans };
    const meals = [...(u[dayIdx] || [])];
    const meal = meals.find(m => m.id === mealId);
  ...
  const MacroInputField = ({ label, value, onChange }) => (
    <div>
  ...
  const MacroCardGroup = ({ type, label, icon: Icon, color }) => (
    <div className="bg-slate-700/50 rounded-xl p-5 border border-slate-600/50">
      ...
      <div className="grid grid-cols-2 gap-3">
        {Object.keys(MACRO_LABELS).map(f => (
                      ...
                      {cfg.useCustomMacros && (
                        <div className="px-4 pb-3 pt-1 border-t border-slate-700/30">
                          <div className="grid grid-cols-4 gap-2">
                            {Object.keys(MACRO_LABELS_SHORT).map(f => (
        ...
        {activeTab === 'ingredients' && (
          <div className="space-y-5">
            ...
            <div className="bg-slate-800/60 backdrop-blur rounded-xl p-5 border border-slate-700/50">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                <select value={newIngredient.category}
                  ...
                  {['Protein', 'Carbs', 'Fats', 'Veggies', 'Fruit'].map(c => <option key={c}>{c}</option>)}
                </select>
            ...
            <div className="bg-slate-800/60 backdrop-blur rounded-xl border border-slate-700/50 overflow-hidden">
              {['Protein', 'Carbs', 'Fats', 'Veggies', 'Fruit'].map(cat => {
                const items = favoriteIngredients.filter(x => x.category === cat);
                if (items.length === 0) return null;
                const open = expandedIngCat === cat;
                const catColors = { Protein: 'text-red-400', Carbs: 'text-blue-400', Fats: 'text-yellow-400', Veggies: 'text-green-400', Fruit: 'text-pink-400' };
                return (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {Object.keys(mealPlans).length > 0 && (
                <span className="text-xs text-slate-500">{Object.keys(mealPlans).map(i => DAYS_SHORT[i]).join(', ')}</span>
              )}
            </div>
